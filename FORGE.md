...
- **Files:** 15 (+2267/-0)
- **Duration:** 949ss
- **Approach:** Built the reporting domain layer in three tiers: (1) DB migration 009 creating report_definitions and export_jobs with full RLS (ENABLE/FORCE, ALL-command USING+WITH CHECK policy, tenant_id-leading indexes on all three indexes per table, FK with ON DELETE SET NULL); (2) frozen ReportFieldCatalog mapping 11 dimensions and 8 metrics to catalog entries with groupByExpr, filterColumnExpr, allowedOperators, valueSchema, requiresJoins; (3) compileReportQuery() building fully parameterized aggregate SQL — tenant predicate unconditional, org-scope predicate for scoped roles, filter AST compiled via @opsninja/filter-compiler with placeholder offset renumbering, GROUP BY ordinals, LIMIT at ROW_CAP+1. validateReportFilterAst() validates against the DIMENSION_CATALOG with REPORT_FILTER_* error codes including reversed-between detection. computeQuerySignature() includes orgScopeVersion for cache invalidation. Unit tests cover all metrics/dimensions, injection safety (payload in params only), signature determinism, catalog retirement. Integration tests use a 12-month two-tenant seed and verify aggregate counts and org-scope isolation.
...
- **Files:** 23 (+1672/-0)
- **Duration:** 877ss
- **Approach:** Implemented the full CSAT loop: (1) Migration 010 creates csat_surveys (tenant_id leading key, unique (tenant_id, ticket_id), unique token_hash, partial index on responded_at, ENABLE/FORCE RLS ALL-command policy) plus expand-only org config columns (csat_enabled, csat_fatigue_hours, csat_expiry_days). (2) CsatTokenService: crypto.randomBytes(32) base64url for transport, SHA-256 hex stored as token_hash, timingSafeEqual constant-time comparison, injectable clock for expiry testing. (3) CsatTokenGuard: rate-limits per token-hash and per-IP via Redis fixed-window before DB lookup; uses SET LOCAL row_security=off in a transaction for the cross-tenant token_hash→tenant_id lookup, attaches survey to request[CSAT_SURVEY_KEY]. (4) CsatService.submit(): single conditional UPDATE WHERE responded_at IS NULL RETURNING id for atomic single-use enforcement; 0 rows = 409. (5) CsatController: GET /api/v1/csat/:token returns allow-listed projection only, POST /api/v1/csat/:token with Zod strict DTO; aggregation endpoint GET /api/v1/reporting/csat/summary gated by TICKETS_READ. (6) CsatAggregationService: TenantScopedReplicaRunner with 30s statement timeout, zero-state returns structured zeros. (7) CsatDispatchHandler in notification worker: validates ticket.resolved event, checks org csat_enabled, fatigue window (csat_fatigue_hours), reopen suppression (24h unanswered survey), idempotent INSERT onConflictDoNothing on (tenant_id, ticket_id), renders csat_survey template with per-score URL links, marks delivered=true on send success. (8) Log redactor extended with csatToken, csat_token, rawToken, tokenHash, token_hash fields. (9) App.module.ts wired CsatModule.
...
- **Files:** 35 (+2213/-2)
- **Duration:** 924ss
- **Approach:** Implemented the full webhook delivery data plane as three integrated layers: (1) a shared @opsninja/webhooks package with canonical-payload deterministic serialiser, HMAC-SHA256 signature builder/verifier with dual-rotation support, SSRF re-validator, webhook dispatcher using Node 22 global fetch with AbortController timeouts, and retry classifier with [1,2,4,8,60,900]s backoff; (2) a standalone apps/workers/webhook-worker NestJS consumer that polls SQS, applies Redis per-endpoint concurrency semaphore and per-tenant token bucket Lua scripts, decrypts signing secrets per-delivery, records idempotent attempt rows, and auto-disables after 20 consecutive failures via atomic SQL CASE UPDATE; (3) two API endpoints on the existing webhooks module — cursor-paginated delivery history and replay that re-enqueues the stored canonical payload with a new attempt number.
