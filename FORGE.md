...
## WO-010: User Story: WO-010 - Enterprise OIDC login with PKCE for internal staff
- **Status:** completed
- **Commit:** `7588f96`
- **Files:** 13 (+1617/-36)
- **Duration:** 1110ss
- **Approach:** Built on top of WO-005 (OIDC stack). Added per-tenant IdP connection configuration via idp_connections table (migration 0010, RLS-enabled) and a SecretsProvider port so client_secret is resolved at runtime from Secrets Manager and never stored in the DB. Added JwksCacheService (standalone, single-flight, negative-kid-cache DoS guard) and UserProvisioningService that upserts staff on (tenant_id, external_subject) — the stable OIDC sub claim — enforces email_verified, allowed_email_domains, disabled-user and zero-roles guards, and returns a fully resolved AuthenticatedPrincipal. Updated OidcService so buildAuthorizationUrl() stores the S256 challenge (not raw verifier) in Redis and returns the raw codeVerifier to the caller; exchangeCode() accepts codeVerifier, validates S256 match, and adds email_verified enforcement. Updated AuthController: handleLogin() now returns JSON {authorizationUrl, state, codeVerifier} (200 not 302); handleCallback() accepts codeVerifier, delegates to UserProvisioningService when available, and returns the WO-010 response shape {user, accessToken, expiresIn} with all required error codes. Added unit tests (oidc-claims.test.ts) covering PKCE generation, S256 challenge storage, state single-use, S256 mismatch, domain check, email_verified, disabled/no-roles. Added integration tests (oidc.e2e-spec.ts) using Testcontainers + MockOidcProvider covering full round-trip, replayed state, tampered verifier, expired token, email_verified:false, and IdpConnectionRepository caching.

## WO-018: User Story: WO-018 - Build Accessible Core UI Primitives on Radix
- **Status:** completed
- **Commit:** `eef8cf1`
- **Files:** 47 (+2657/-2)
- **Duration:** 686ss
- **Approach:** Delivered the full WO-018 core primitive library in packages/ui-kit as thin, token-driven wrappers around Radix UI primitives. Each interactive component is marked 'use client' for Next.js 15 App Router compatibility. Styling uses CVA variant maps referencing only semantic Tailwind token utilities (bg-accent, text-primary, ring-focus, bg-danger, etc.) from WO-017 — no raw colour or hex values. All overlay/interactive behaviour (focus traps, portals, ARIA wiring, keyboard interaction, focus restoration) is delegated to Radix primitives. FormField owns useId-based id generation and wires aria-invalid and aria-describedby onto its children. Button/IconButton support asChild via Radix Slot for Next Link composition. Loading Button sets aria-busy=true. Alert uses role=alert for error variants and role=status for all others, always carries an icon alongside colour. Pagination uses cursor-shaped props with limit clamped at 100. Avatar falls back from image → initials → neutral icon glyph. A cn() utility wraps tailwind-merge for safe class composition without pulling in clsx. Icons are re-exported from lucide-react through src/icons/index.ts so consumers have no direct lucide dependency. Vitest + Testing Library + vitest-axe tests cover all 25 components with axe assertions. Fixtures provide offline test data for all components.

## WO-036: User Story: WO-036 - Multi-Level Ticket Categorization Engine
- **Status:** completed
- **Commit:** `353779c`
- **Files:** 11 (+1983/-5)
- **Duration:** 701ss
- **Approach:** Implemented the multi-level ticket categorisation engine as a self-contained domain module under apps/api/src/modules/tickets/categories/. Used an expand-only migration (0036) to add slug, depth, sort_order, is_active columns to the existing categories table (from migration 0001) plus RLS tenant_isolation policy and supporting indexes. The path-builder module provides pure, testable functions for slug generation, path computation, cycle detection and depth validation. The repository layer uses a recursive CTE for atomic subtree path rewrites with row locks. The service layer coordinates validation (cycle detection, depth limits, sibling uniqueness) and exposes CategoriesPort (resolveById/resolvePaths) for cross-module consumption by the tickets service, saved-view compiler and report builder. A version-keyed in-process cache with an injectable VersionStore port (Redis in production, null in tests) avoids re-reading the full tree on every ticket serialisation. The controller is framework-agnostic (returns plain objects) with RBAC enforcement (admin-only mutations). A DevOps taxonomy fixture with 15 deterministic-UUID nodes (4 roots, depth-1 children, EC2 Instances at depth 2) is committed for reuse across queue and reporting tests. Unit tests cover path-builder and service logic against mocked repositories. Integration tests use Testcontainers PostgreSQL 16.
